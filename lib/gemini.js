 ...
 */
export async function generateAllQuestions(role, difficulty, count) {
  const prompt = `You are a senior technical interviewer at a top tech company.
Generate exactly ${count} unique interview questions for a ${role} position at ${difficulty} difficulty level.

Rules:
- Return ONLY a valid JSON array of strings
- No numbering, no explanation, no markdown
- Each question must be unique and non-repetitive
- For Hard: system design or advanced concepts
- For Medium: practical problem-solving
- For Easy: fundamentals and basic concepts
