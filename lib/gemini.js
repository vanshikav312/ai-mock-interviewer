...
/**
 * Retries an async function with exponential backoff when Gemini returns
 * 503 (overloaded) or 429 (rate-limited).
 */
async function withRetry(fn, maxAttempts = 4, baseDelayMs = 1500) {
  ...
  for (let attempt = 1; attempt <= maxAttempts; attempt++) {
    try {
      ...
      // Use Gemini's retryDelay if provided, otherwise fall back to exponential backoff
      ...
      try {
        ...
        if (retryInfo?.retryDelay) {
          // retryDelay is a string like "9s" or "1.5s"
          const seconds = parseFloat(retryInfo.retryDelay.replace('s', ''));
          if (!isNaN(seconds)) delay = seconds * 1000 + 500; // add 500ms buffer
        }
      } catch (_) { /* ignore parse errors, use default delay */ }
